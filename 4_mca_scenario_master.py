...
class OfflineMissionPlanner:
    def __init__(self):
        ...
        self.wp_distance_var = tk.DoubleVar(value=2.0) # Khởi tạo khoảng cách mặc định là 2.0 mét
    ...
    def _build_gui(self):
        ...
        # CHÈN THÊM NÚT SỬA ĐOẠN ĐANG CHỌN TẠI ĐÂY
        ttk.Button(btn_f1, text="⚙️ Sửa đoạn chọn", command=self.edit_selected_segment).pack(side="left", expand=True, padx=2)
        ...
        # --- CHÈN THÊM KHUNG ĐIỀU CHỈNH KHOẢNG CÁCH MÉT TRUNG GIAN ---
        dist_f = ttk.Frame(l_frame)
        dist_f.pack(fill="x", pady=2)
        ttk.Label(dist_f, text="Khoảng cách điểm trung gian (m):", font=('Arial', 9, 'italic')).pack(side="left", padx=2)
        ttk.Entry(dist_f, textvariable=self.wp_distance_var, width=8).pack(side="right", padx=2)
        # -------------------------------------------------------------
        ...
        # CHÈN THÊM NÚT LƯU FILE SPAWN POINT SẠCH NGUYÊN BẢN TẠI ĐÂY
        ttk.Button(io_f, text="💾 Lưu SP Sạch", command=self.save_pure_spawn_points).pack(side="left", expand=True, padx=2)
    ...
    def calculate_target_speed_profile(self, df_trajectory):
        """
        Tự động điều chỉnh vận tốc đặt tại các điểm trung gian.
        Nếu đi vào đoạn cua (YawRate lớn), tốc độ đặt sẽ tự giảm để đảm bảo gia tốc ngang an toàn.
        """
        dt = 0.05
        # Tính bán kính cong hoặc tốc độ quay góc (Yaw Rate)
        dX = np.gradient(df_trajectory['X'])
        dY = np.gradient(df_trajectory['Y'])
        yaw = np.unwrap(np.arctan2(dY, dX))
        yaw_rate = np.abs(np.gradient(yaw, dt))
        
        speed_profile = []
        for idx, row in df_trajectory.iterrows():
            v_orig = row['Speed_kmh'] / 3.6 # Quy đổi về m/s
            
            # Nếu tốc độ quay góc > 0.1 rad/s (đang vào khúc cua) -> Hạ tốc độ mục tiêu xuống
            if yaw_rate[idx] > 0.1:
                # Công thức động lực học giới hạn gia tốc ly tâm: v = sqrt(a_lat_max / curvature)
                v_curve = math.sqrt(3.0 / max(0.01, yaw_rate[idx] / max(1.0, v_orig)))
                v_target = min(v_orig, v_curve)
            else:
                v_target = v_orig
                
            speed_profile.append(v_target)
            
        # Làm mịn hồ sơ vận tốc bằng bộ lọc trung bình trượt (tránh xe bị giật cục)
        df_trajectory['Target_V_ms'] = pd.Series(speed_profile).rolling(window=10, min_periods=1, center=True).mean()
    ...
    def save_spawn_list(self):
        """Lưu danh sách lộ trình đầy đủ thông số (Spawn Index, Vận tốc, Loại đoạn) để phục vụ nạp lại kịch bản"""
        if not self.route_segments: 
        ...
        if path:
            ...
            messagebox.showinfo("Thành công", "Đã xuất cấu hình danh sách lộ trình thành công. Thầy có thể nạp lại qua nút 'Nạp Lộ trình'.")
    ...
    # --- THAY THẾ TOÀN BỘ HÀM CŨ TẠI ĐÂY BẰNG ĐOẠN CODE DƯỚI ĐÂY ---
    def export_trajectory_time(self):
        try:
            if not self.is_route_finalized: 
                return messagebox.showwarning("Lỗi", "Vui lòng '📍 CHỐT ĐIỂM CUỐI'.")
            ...
            if len(full_path) < 3: 
                return messagebox.showwarning("Lỗi", "Quãng đường quá ngắn.")
            ...
            # GIAI ĐOẠN 1: TÍNH TOÁN ĐỘNG LỰC HỌC THEO THỜI GIAN (GIỮ NGUYÊN GỐC TÍNH TOÁN CỦA THẦY)
            ...
            if len(raw_data) < 2: 
                return messagebox.showwarning("Lỗi", "Dữ liệu quá ngắn.")
            
            df_time = pd.DataFrame(raw_data, columns=['Time_s', 'X', 'Y', 'Speed_kmh', 'Accel_X_ms2', 'Jerk'])
            
            # Khử góc vuông hình học ly tâm
            df_time['X_smooth'] = df_time['X'].rolling(window=15, min_periods=1, center=True).mean()
            df_time['Y_smooth'] = df_time['Y'].rolling(window=15, min_periods=1, center=True).mean()
            
            dX = np.gradient(df_time['X_smooth'])
            dY = np.gradient(df_time['Y_smooth'])
            yaw_unwrapped = np.unwrap(np.arctan2(dY, dX))
            yaw_rate = np.gradient(yaw_unwrapped, dt)
            
            df_time['Yaw_rad'] = yaw_unwrapped
            df_time['YawRate_rad_s'] = pd.Series(yaw_rate).rolling(window=20, min_periods=1, center=True).mean()
            
            a_lat = (df_time['Speed_kmh'] / 3.6) * df_time['YawRate_rad_s']
            df_time['Accel_Y_ms2'] = np.clip(pd.Series(a_lat).rolling(window=10, min_periods=1).mean(), -10.0, 10.0)

            # --- GIAI ĐOẠN 2: NỘI SUY KHÔNG GIAN BĂM LƯỚI ĐIỂM DỰA TRÊN GUI ---
            KHOANG_CACH_MONG_MUON = max(0.1, self.wp_distance_var.get())  # Lấy dữ liệu động từ GUI, chặn lỗi <= 0
            
            dx_flat = np.gradient(df_time['X'])
            dy_flat = np.gradient(df_time['Y'])
            ds_flat = np.sqrt(dx_flat**2 + dy_flat**2)
            s_accumulated = np.cumsum(ds_flat)
            
            total_length = s_accumulated.iloc[-1] if isinstance(s_accumulated, pd.Series) else s_accumulated[-1]
            s_new = np.arange(0, total_length, KHOANG_CACH_MONG_MUON)
            
            df_resampled = pd.DataFrame()
            df_resampled['X'] = np.interp(s_new, s_accumulated, df_time['X'])
            df_resampled['Y'] = np.interp(s_new, s_accumulated, df_time['Y'])
            df_resampled['Time_s'] = np.interp(s_new, s_accumulated, df_time['Time_s'])
            df_resampled['Speed_kmh'] = np.interp(s_new, s_accumulated, df_time['Speed_kmh'])
            df_resampled['Accel_X_ms2'] = np.interp(s_new, s_accumulated, df_time['Accel_X_ms2'])
            df_resampled['Accel_Y_ms2'] = np.interp(s_new, s_accumulated, df_time['Accel_Y_ms2'])
            df_resampled['Yaw_rad'] = np.interp(s_new, s_accumulated, df_time['Yaw_rad'])
            df_resampled['YawRate_rad_s'] = np.interp(s_new, s_accumulated, df_time['YawRate_rad_s'])
            df_resampled['Jerk'] = np.interp(s_new, s_accumulated, df_time['Jerk'])
            
            df_resampled['Target_V_ms'] = df_resampled['Speed_kmh'] / 3.6
            
            # Điều hướng duy nhất sang bảng Editor can thiệp vận tốc đặt
            self.open_waypoint_speed_editor(df_resampled)
            
        except Exception as e: 
            import traceback; print(traceback.format_exc())
            messagebox.showerror("Lỗi Thuật Toán", f"Sự cố xuất dữ liệu:\n{str(e)}")

    def open_waypoint_speed_editor(self, df_trajectory):
        """Mở cửa sổ cấu hình vận tốc trực quan cho các điểm trung gian"""
        # Tạo bản sao độc lập để loại bỏ hoàn toàn cảnh báo SettingWithCopyWarning của Pandas
        df_trajectory = df_trajectory.copy()
        
        editor_window = tk.Toplevel(self.root)
        editor_window.title("Bộ can thiệp vận tốc Điểm trung gian (Waypoints Speed Editor)")
        editor_window.geometry("750x600")
        
        # Ép cửa sổ luôn nổi lên trên giao diện chính tránh bị che khuất
        editor_window.lift()
        editor_window.attributes('-topmost', True)
        editor_window.focus_force()
        
        ttk.Label(editor_window, text="Danh sách điểm nội suy (cách đều 1.0m). Thầy click đúp vào ô 'V Can Thiệp' để sửa:", 
                  font=('Arial', 10, 'bold')).pack(pady=10)

        frame_table = ttk.Frame(editor_window)
        frame_table.pack(fill="both", expand=True, padx=10, pady=5)
        
        scroll_y = ttk.Scrollbar(frame_table)
        scroll_y.pack(side="right", fill="y")

        tree_wp = ttk.Treeview(frame_table, columns=("id", "x", "y", "current_v", "new_v"), 
                               show="headings", yscrollcommand=scroll_y.set)
        scroll_y.config(command=tree_wp.yview)
        
        tree_wp.heading("id", text="STT (Mét)")
        tree_wp.heading("x", text="Tọa độ X")
        tree_wp.heading("y", text="Tọa độ Y")
        tree_wp.heading("current_v", text="V Động lực học (km/h)")
        tree_wp.heading("new_v", text="V Can thiệp (km/h)")
        
        tree_wp.column("id", width=80, anchor="center")
        tree_wp.column("x", width=120, anchor="center")
        tree_wp.column("y", width=120, anchor="center")
        tree_wp.column("current_v", width=150, anchor="center")
        tree_wp.column("new_v", width=150, anchor="center")
        tree_wp.pack(fill="both", expand=True)

        for idx, row in df_trajectory.iterrows():
            tree_wp.insert("", "end", iid=str(idx), values=(
                idx, 
                f"{row['X']:.2f}", 
                f"{row['Y']:.2f}", 
                f"{row['Speed_kmh']:.1f}", 
        ...
        def on_double_click(event):
            item_id = tree_wp.identify_row(event.y)
            column = tree_wp.identify_column(event.x)
            
            if column == '#5' and item_id:
                x, y, w, h = tree_wp.bbox(item_id, column)
                entry_edit = ttk.Entry(tree_wp, width=10)
                entry_edit.insert(0, tree_wp.item(item_id, "values")[4])
                entry_edit.place(x=x, y=y, width=w, height=h)
                entry_edit.focus()
                
                def save_cell(e):
                    try:
                        val = float(entry_edit.get())
                        vals = list(tree_wp.item(item_id, "values"))
                        vals[4] = f"{val:.1f}" 
                        tree_wp.item(item_id, values=vals)
                    except ValueError:
                        pass
                    entry_edit.destroy()
                    
                entry_edit.bind("<Return>", save_cell)
                entry_edit.bind("<FocusOut>", lambda e: entry_edit.destroy())

        tree_wp.bind("<Double-1>", on_double_click)

        # KHUNG CÔNG CỤ CAN THIỆP NHANH HÀNG LOẠT
        frame_batch = ttk.LabelFrame(editor_window, text=" Can thiệp nhanh hàng loạt (Batch Edit) ")
        frame_batch.pack(fill="x", padx=10, pady=10)
        
        ttk.Label(frame_batch, text="Từ Điểm (m):").grid(row=0, column=0, padx=5, pady=5)
        entry_from = ttk.Entry(frame_batch, width=6); entry_from.grid(row=0, column=1, padx=5, pady=5)
        
        ttk.Label(frame_batch, text="Đến Điểm (m):").grid(row=0, column=2, padx=5, pady=5)
        entry_to = ttk.Entry(frame_batch, width=6); entry_to.grid(row=0, column=3, padx=5, pady=5)
        
        ttk.Label(frame_batch, text="Set Tốc độ (km/h):").grid(row=0, column=4, padx=5, pady=5)
        entry_v_batch = ttk.Entry(frame_batch, width=8); entry_v_batch.grid(row=0, column=5, padx=5, pady=5)
        
        def apply_batch():
            try:
                start_i = int(entry_from.get())
                end_i = int(entry_to.get())
                v_target = float(entry_v_batch.get())
                for i in range(start_i, end_i + 1):
                    if tree_wp.exists(str(i)):
                        vals = list(tree_wp.item(str(i), "values"))
                        vals[4] = f"{v_target:.1f}"
                        tree_wp.item(str(i), values=vals)
            except Exception:
                messagebox.showerror("Lỗi", "Vui lòng nhập số nguyên cho Điểm và số thực cho Tốc độ!", parent=editor_window)

        ttk.Button(frame_batch, text="⚡ Áp dụng cho đoạn", command=apply_batch).grid(row=0, column=6, padx=10, pady=5)

        def confirm_and_close():
            # 1. Thu thập dữ liệu can thiệp thô từ giao diện Treeview
            for idx in df_trajectory.index:
                if tree_wp.exists(str(idx)):
                    new_v_kmh = float(tree_wp.item(str(idx), "values")[4])
                    df_trajectory.loc[idx, 'Speed_kmh'] = new_v_kmh
                    df_trajectory.loc[idx, 'Target_V_ms'] = new_v_kmh / 3.6

            # =========================================================================
            # 2. THUẬT TOÁN HỒ SƠ VẬN TỐC HẠN CHẾ GIA TỐC VÀ JERK CHỐNG RUNG SÀN 6-DOF
            # =========================================================================
            a_max = 3.0  # Gia tốc cực đại thiết lập (m/s^2)
            j_max = 2.0  # Jerk cực đại thiết lập cho hệ thống cơ điện tử (m/s^3)
            ds = max(0.1, self.wp_distance_var.get())
            ...
            # Trạng thái ban đầu tại điểm xuất phát kịch bản
            v_curr = df_trajectory.loc[0, 'Target_V_ms']
            a_curr = 0.0
            t_curr = 0.0

            v_filtered.append(v_curr)
            a_filtered.append(a_curr)
            j_filtered.append(0.0)
            t_filtered.append(t_curr)

            # Mô phỏng tích phân tiến giới hạn vi phân động lực học dọc (Surge Control)
            for i in range(1, len(df_trajectory)):
                v_tgt = df_trajectory.loc[i, 'Target_V_ms']
                
                # Tính toán chu kỳ thời gian thực tế dt dựa trên vận tốc trung bình hành trình
                v_avg = max(0.5, (v_curr + v_tgt) / 2.0)
                dt = ds / v_avg

                # Gia tốc cần thiết để bám theo vận tốc can thiệp mới
                a_req = (v_tgt - v_curr) / dt
                
                # Ràng buộc bậc 3 - Giới hạn tốc độ thay đổi gia tốc (Jerk Max Constraint)
                j_req = (a_req - a_curr) / dt
                j_req = max(-j_max, min(j_max, j_req))
                
                # Cập nhật trạng thái gia tốc (Bậc 2 - Accel Constraint)
                a_curr += j_req * dt
                a_curr = max(-a_max, min(a_max, a_curr))
                
                # Tích phân vận tốc thực tế
                v_curr += a_curr * dt
                v_curr = max(0.1, v_curr)  # Ngăn chặn vận tốc về không gây lỗi chia cho 0 ở bước kế tiếp
                t_curr += dt

                v_filtered.append(v_curr)
                a_filtered.append(a_curr)
                j_filtered.append(j_req)
                t_filtered.append(t_curr)

            # Gán lại các mảng động học mịn đã được triệt tiêu rung chấn vào hệ thống DataFrame
            df_trajectory.loc[:, 'Target_V_ms'] = v_filtered
            df_trajectory.loc[:, 'Speed_kmh'] = np.array(v_filtered) * 3.6
            df_trajectory.loc[:, 'Accel_X_ms2'] = a_filtered
            df_trajectory.loc[:, 'Jerk'] = j_filtered
            df_trajectory.loc[:, 'Time_s'] = t_filtered

            # 3. Đồng bộ lại Động lực học ngang (Sway & Yaw Rate) theo biến thiên tốc độ mới
            dX = np.gradient(df_trajectory['X'])
            dY = np.gradient(df_trajectory['Y'])
            yaw = np.unwrap(np.arctan2(dY, dX))
            df_trajectory.loc[:, 'Yaw_rad'] = yaw
            
            # Tính toán vi phân góc quay dựa trên dải thời gian mịn mới tạo
            dt_mean = max(0.01, np.mean(np.gradient(t_filtered)))
            yaw_rate = np.gradient(yaw, dt_mean)
            df_trajectory.loc[:, 'YawRate_rad_s'] = pd.Series(yaw_rate).rolling(window=10, min_periods=1, center=True).mean()
            
            # Tính toán lại lực ly tâm ngang và cắt ngưỡng an toàn cơ khí sàn
            a_lat = (df_trajectory['Target_V_ms']) * df_trajectory['YawRate_rad_s']
            df_trajectory.loc[:, 'Accel_Y_ms2'] = np.clip(pd.Series(a_lat).rolling(window=10, min_periods=1, center=True).mean(), -10.0, 10.0)

            editor_window.attributes('-topmost', False)
            editor_window.destroy()
            
            cols = ['Time_s', 'X', 'Y', 'Yaw_rad', 'Target_V_ms', 'Speed_kmh', 'Accel_X_ms2', 'Accel_Y_ms2', 'YawRate_rad_s', 'Jerk']
            self.show_preview_dialog(df_trajectory[cols])

        ttk.Button(editor_window, text="✔️ XÁC NHẬN VÀ CHUYỂN SANG PREVIEW ĐỂ LƯU FILE", style="Accent.TButton", 
                   command=confirm_and_close).pack(fill="x", padx=10, pady=10)

    # --- KHU VỰC TIẾP THEO LÀ HÀM PREVIEW GỐC CỦA THẦY ---
    ...
    def show_preview_dialog(self, df):
       ...
       # Gọi hàm tính toán tốc độ mục tiêu biến thiên
        df_out = self.calculate_target_speed_profile(df)
        
        # Cấu trúc các cột xuất ra file CSV
        # CARLA Controller sẽ đọc file này, tìm dòng tương thích vị trí và ép xe chạy theo Target_V_ms
        cols_for_carla = ['Time_s', 'X', 'Y', 'Yaw_rad', 'Target_V_ms', 'Accel_X_ms2']
    # ==========================================================================
    ...
    def edit_selected_segment(self):
            #"""Cho phép can thiệp chỉnh sửa thông số Chế độ và Vận tốc đặt của đoạn đang chọn trên Treeview"""
            selected = self.tree.selection()
            if not selected:
                return messagebox.showwarning("Cảnh báo", "Thầy vui lòng click chọn một Đoạn hành trình trong bảng danh sách phía trên!")
            
            item_vals = self.tree.item(selected[0], "values")
            idx_str = item_vals[0]
        
            if "START" in idx_str or "-" in idx_str:
                return messagebox.showwarning("Cảnh báo", "Không thể sửa thông số của điểm khởi hành START!")
            
            try:
                # Tách chuỗi lấy số thứ tự đoạn (Ví dụ: "Đoạn 1" -> 1)
                seg_idx = int(idx_str.split()[-1])
                seg = self.route_segments[seg_idx]
            
                # Đè giá trị cấu hình hiện tại từ các ô nhập liệu trên GUI vào đoạn cấu trúc segment
                seg['type'] = self.ctrl_mode.get()
                seg['val'] = self.target_value.get()
            
                # Cập nhật lại tham số kịch bản mẫu nếu có
                if seg['type'] == 'speed':
                    seg['params'] = {'v': self.target_value.get()}
                else:
                    seg['params'] = {}
                
                # Vẽ lại bản đồ và cập nhật thông tin hiển thị lên danh sách
                self.update_viz()
                messagebox.showinfo("Thành công", f"Đã cập nhật Đoạn {seg_idx} sang Chế độ: {seg['type']} với Giá trị đặt: {seg['val']}")
            except Exception as e:
                messagebox.showerror("Lỗi", f"Không thể chỉnh sửa thông số đoạn: {str(e)}")

    def save_pure_spawn_points(self):
        """YÊU CẦU 1: Lưu file CSV nguyên bản CHỈ chứa thứ tự chuỗi các Spawn Point đã chọn (Không kèm tốc độ/gia tốc)"""
        if not self.route_segments: 
            return messagebox.showwarning("Lỗi", "Chưa có lộ trình để trích xuất.")
        ...
        for i, seg in enumerate(self.route_segments):
            ...
            data_pure.append({
                'Order': i,               # Thứ tự các chặng di chuyển
                'Spawn_Index': idx,       # Mã định danh ID của điểm trong CARLA
                'X': sp['x'], 
                'Y': sp['y'], 
                'Z': sp['z']
            })
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"pure_spawn_points_{self.map_name}.csv",
            filetypes=[("CSV Files", "*.csv")]
        )
        if path:
            pd.DataFrame(data_pure).to_csv(path, index=False)
            messagebox.showinfo("Thành công", f"Đã xuất file Spawn Points nguyên bản sạch tại:\n{path}")
